package io.migrant.pir.application;
...
public class PIRDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_TABLE_USERS = "CREATE TABLE users(user_id TEXT PRIMARY KEY NOT NULL, type TEXT, email TEXT, _v TEXT, activated INT, last_login TEXT, created TEXT);";
    ...
    private static final String SELECT_TOKEN = "SELECT * FROM token LIMIT 1";
    private static final String DELETE_TOKEN = "DELETE * FROM token";
    ...
    public Parent getParent() {
        ...
        if (csr != null) {
            Log.i(ApplicationController.TAG, "getParent() cursor not null");
            if (csr.moveToFirst()) {
    ...
    public String getToken() {
        String token = "";
        SQLiteDatabase db = getDbInstance();
        Cursor csr = db.rawQuery(SELECT_TOKEN, null);
        if (csr != null) {
            if (csr.moveToFirst()) {
    ...
    public boolean deleteToken(){
        SQLiteDatabase db = getDbInstance();
        return db.delete(TOKEN_TABLE, null, null) == 1;
